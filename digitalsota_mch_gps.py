...
def bitarray_to_str(arr):
    ...
    if (hashsum == send_data_tlm[5]):
        if (send_data_tlm[0]==0xAA):
            temp1 = (send_data_tlm[1]<<24) | (send_data_tlm[2]<<16) | (send_data_tlm[3]<<8) | send_data_tlm[4]
            
            if (temp1&0x80000000):
                temp1= -0x100000000+temp1
            

            telemetryfile = open(tlmname+'1', 'a')
            data_aa = '\t AA ' + str(float(temp1)/1000000.0) + '\r\n'
        ...
        if (send_data_tlm[0]==0xAB):
            ...
            data_aa = '\t AB ' + str(float(temp1)/1000000.0) + '\r\n'
            telemetryfile.write(datetime.now().isoformat() + data_aa)
            #print 'AA ' , s_t
            telemetryfile.close()
